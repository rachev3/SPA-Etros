...
import { useUpdateMatch } from "../../../api/matchApi";

const EditMatchModal = ({ isOpen, onClose, onSuccess, match }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await update(match._id, matchData);
      onSuccess();
    } catch (err) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  disabled={loading}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 disabled:bg-gray-100"
                  value={matchData.status || "upcoming"}
                  onChange={(e) =>
              ...
              <>
                ...
                <div>
                  ...
                  <select
                    disabled={loading}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 disabled:bg-gray-100"
                    value={matchData.result || "Pending"}
                    onChange={(e) =>
                ...
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {Object.entries(matchData.teamStats || {}).map(
                      ([key, value]) => (
                        <div key={key}>
                          <label className="block text-sm font-medium text-gray-700 mb-1">
